...
export const CURRENT_INSTALL_SPEC =
  "https://github.com/chekdata/chek-cli/archive/refs/heads/main.tar.gz?download=1";
...
export function detectReplyIntent(
  ...
  const corpus = [task.payload.postTitle || "", task.payload.messageContent || ""]
    ...
    .toLowerCase();
  if (
    /openclaw|chek cli|chek-cli|memor upload|memor-upload|\/chek-setup|设备绑定|浏览器授权|完成授权|完成绑定/.test(
      corpus,
    )
  ) {
